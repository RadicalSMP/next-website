...
// ─── 获取评分规则 ─────────────────────────────────────────
export async function getReviewScoringRules(formSlug: string = "join-application") {
    "use cache";
    cacheTag(CACHE_TAGS.REVIEW_RULES);
    cacheLife("hours");
...
export async function getReviewSubmissions(
    ...
    "use cache";
    cacheTag(CACHE_TAGS.REVIEW_SUBMISSIONS);
    cacheLife("default");
